...
//setup SD card
  ...
  //write down the date (year / month / day         prints only the start, so if the logger runs for sevenal days you only findt the start back at the begin.
    ...
    dataFile.println(" ");
    dataFile.println("Weight(grams)              Time");
    dataFile.close();
}
...
//read the time
  ...
  if (dataFile) {
     ...
     // read three sensors and append to the string:
       val = cell.read();
       reading = ((val-8427701)/-19302.f * 180); // (((MaxNew-MinNew)/(MaxOld-MinOld))*(Value-MaxOld))+MaxNew here MinNew=0, MaxNew=100, MaxOld=723, MinOld=431 i.e after subtracting from 1000
       reading = -(79225-reading);
